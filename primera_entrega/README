...
1. Archivo `test_api.ps1` en la raíz:
   ...
   function Get-AllProducts { Invoke-RestMethod -Method GET -Uri "$BaseUrl/api/products" }

   function Find-ProductIdByCode([string]$code) {
     $all = Get-AllProducts
     foreach ($p in $all.data) { if ($p.code -eq $code) { return $p.id } }
     return $null
   }
   ...
   # Productos base
   $p1Spec = @{
     title="Laptop Pro 14"; description="Equipo portátil de alto rendimiento"; code="LP14-001";
     price=28999.90; status=$true; stock=50; category="computo";
     thumbnails=@("images/lp14-front.jpg","images/lp14-back.jpg")
   }
   $p2Spec = @{
     title="Mouse Inalámbrico"; description="Mouse óptico 1600dpi"; code="MS-001";
     price=399.00; status=$true; stock=200; category="accesorios"; thumbnails=@("images/mouse.jpg")
   }
